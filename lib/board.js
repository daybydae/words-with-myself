...
class Board {
  ...
  handleTileMousedown(e) {
    ...
    this.previousTilePos = $(e.currentTarget).data().pos;
  }
  ...
  isValidSelection(pos) {
    ...
    if (
      (((pos[0] - 1) === this.previousTilePos[0]) ||
      ((pos[0] + 1) === this.previousTilePos[0]) ||
      ((pos[0]) === this.previousTilePos[0])) &&
      (((pos[1] - 1) === this.previousTilePos[1]) ||
      ((pos[1] + 1) === this.previousTilePos[1]) ||
      ((pos[1]) === this.previousTilePos[1]))
      ) {
        return true;
    }
    return false;
